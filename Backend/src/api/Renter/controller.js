  ...
  booking.find({ "renter": req.query.renter, "status": "0" }).populate('owner property renter').exec((err, owner) => {
    if (err) {
      ...
      const ress = owner.map(data => {
        return { id: data._id, owner: data.owner.lastName, ownerid: data.owner._id, propertyid: data.property._id, property: data.property.propertyName, renter: data.renter.lastName, houseNo: data.houseNo, roomType: data.roomType, amount: data.amount }
      })
      res.send(ress);
    }
  })
